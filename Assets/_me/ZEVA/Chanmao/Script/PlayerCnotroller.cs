...
public class PlayerCnotroller : MonoBehaviour
{
    [Header("游戏管理器的节点")]
    ...
    void Start()
    {
        zc = GameMaster.GetComponent<ZController>();
    ...
    private void PlayerSlice(int SliceNum)
    {
        ...
        float origin = 0.75f, unlength = 1.5f;
        if (SliceNum > 0)
        {
            origin = 0;
            //判断是否为最后一击                        
        ...
        //将刀光创建出来
        _knite = Instantiate(Knite, new Vector2(transform.position.x,transform.position.y) + SliceDirection * origin, q);

        //根据距离改变斩击的长度
        float scale = (_SliceDistance - unlength) / 5;
    ...
    {
        zc.zTime.TimeUpdate(ref _CoolingTime);
    ...
    {
        zc.zTime.TimeUpdate(ref _IntervalTime,()=>{
            _SliceNum = 0;
            _CoolingTime = CoolingTime;
        });        
    }
